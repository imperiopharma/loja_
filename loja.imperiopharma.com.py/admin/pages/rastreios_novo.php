<?php
/****************************************************************************
 * RASTREIOS AVANÇADO - UM ÚNICO ARQUIVO COM MÁXIMO DE RECURSOS
 ...
 * - Exemplo de agendamento de envio futuro (com CRON)
 ****************************************************************************/

session_start();
// if (!isset($_SESSION['admin_logged'])) { header("Location: login.php"); exit; }

// Se seu cabeçalho HTML global já injeta <head> e <body>, pode retirar esta parte
header('Content-Type: text/html; charset=utf-8');

// Ajuste para seu arquivo de config (conexão PDO etc.)
...
if($action==='send_whatsapp') {
    ...
    if($res['httpCode']===200){
        ...
        echo "<script>alert('Mensagem enviada com sucesso!'); window.location='?tab=matched';</script>";
    } else {
        echo "<script>alert('Falha ao enviar (HTTP {$res['httpCode']}).'); window.location='?tab=matched';</script>";
    }
    exit;
}
...
if($action==='send_whatsapp_bulk' && $_SERVER['REQUEST_METHOD']==='POST' && $ENABLE_BULK_WHATSAPP){
    ...
    if(!is_array($ids) || count($ids)===0){
        echo "<script>alert('Nenhum pedido selecionado!'); window.location='?tab=matched';</script>";
        exit;
    }
    ...
    echo "<a href='?tab=matched'>Voltar</a>";
    echo "</div>";
    exit;
}
...
if($action==='edit'){
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        echo "<script>window.location='?tab=matched';</script>";
        exit;
    }
    ?>
    <!DOCTYPE html>
    ...
    <head>
      <meta charset="utf-8">
      <title>Editar Tracking #<?= $orderId ?></title>
    ...
    <div class="container my-4" style="max-width:600px;">
      ...
      <form method="POST">
        ...
        <a href="?tab=matched" class="btn btn-secondary">Cancelar</a>
      </form>
    </div>
    </body>
    </html>
...
if($action==='update_messages' && $_SERVER['REQUEST_METHOD']==='POST'){
    ...
    echo "<script>alert('Mensagens atualizadas em {$countUp} pedidos!'); window.location='?tab=unmatched';</script>";
    exit;
}
...
if($action==='import_process' && $_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($texto===''){
        exit("<p>Texto vazio. <a href='?tab=import'>Voltar</a></p>");
    }
    $res=parseImport($pdo, $texto, $formato);
    if($res['error']){
        exit("<p>Erro: {$res['error']} <a href='?tab=import'>Voltar</a></p>");
    }
    $arrOK=$res['ok'];
    $arrFail=$res['fail'];
    ?>
    ...
    <head>
      <meta charset="utf-8">
      <title>Resultado Import</title>
      <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
      <style>
        .details-match { display:none; background:#f8f9fa; padding:6px; margin-top:6px; }
      </style>
    </head>
    <body class="bg-light">
    <div class="container my-4">
      <h3>Resultado do Import</h3>
      <form method="POST" action="?action=save_import_results">
        <?php if(!empty($arrOK)): ?>
        ...
        <p class="mt-3">
          <a href="?tab=import" class="btn btn-secondary">Voltar</a>
        </p>
      </form>
    </div>
    <script>
...
if($action==='save_import_results' && $_SERVER['REQUEST_METHOD']==='POST'){
    ...
    echo "<script>alert('Salvo rastreio em {$countOk} associações.'); window.location='?tab=unmatched';</script>";
    exit;
}
...
<head>
  <meta charset="UTF-8">
  <title>Rastreios Avançado - Painel</title>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
  <style>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top">
  <div class="container-fluid">
    <a class="navbar-brand" href="?tab=dashboard">Império Pharma</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navMain" aria-controls="navMain" aria-expanded="false" aria-label="Toggle nav">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navMain">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item"><a class="nav-link <?=($tab==='dashboard'?'active':'')?>" href="?tab=dashboard">Dashboard</a></li>
        <li class="nav-item"><a class="nav-link <?=($tab==='unmatched'?'active':'')?>" href="?tab=unmatched">Sem Rastreio</a></li>
        <li class="nav-item"><a class="nav-link <?=($tab==='matched'?'active':'')?>" href="?tab=matched">Com Rastreio</a></li>
        <li class="nav-item"><a class="nav-link <?=($tab==='import'?'active':'')?>" href="?tab=import">Importar</a></li>
        <li class="nav-item"><a class="nav-link <?=($tab==='logs'?'active':'')?>" href="?tab=logs">Logs Envio</a></li>
        <li class="nav-item"><a class="nav-link <?=($tab==='config'?'active':'')?>" href="?tab=config">Configurações</a></li>
      </ul>
    </div>
  </div>
</nav>
...
<?php if($tab==='dashboard'): ?>
  ...
  ?>
  <h3 class="mb-4">Dashboard / Visão Geral</h3>
  <div class="row">
    <div class="col-md-2">
...
<?php elseif($tab==='unmatched'): ?>
  ...
  <form method="POST" action="?action=update_messages&tab=unmatched">
        ...
        <?php else: ?>
          ...
          <tr>
            ...
            <td>
              ...
              <a href="?action=edit&id=<?=$pid?>&tab=unmatched" class="btn btn-sm btn-primary">Editar Tracking</a>
            </td>
            <td>
...
<?php elseif($tab==='matched'): ?>
  ...
  <form method="POST" action="?action=send_whatsapp_bulk&tab=matched">
  <?php endif;?>
  <div class="table-responsive">
    <table class="table table-bordered align-middle">
      <thead class="table-light">
      ...
      <tbody>
      <?php if(!$rows): ?>
        <tr><td colspan="<?=($ENABLE_BULK_WHATSAPP?9:8)?>" class="text-center text-muted">Nenhum pedido encontrado</td></tr>
      <?php else: ?>
        <?php foreach($rows as $od):
            $pid=(int)$od['id'];
            $cuf=htmlspecialchars(($od['city']??'').'/'.($od['state']??''));
        ...
        <tr class="<?=$rowClass?>">
          ...
          <td>
            <a href="?action=edit&id=<?=$pid?>&tab=matched" class="btn btn-sm btn-primary">Editar</a>
            <a href="?action=undo&id=<?=$pid?>&tab=matched" class="btn btn-sm btn-danger"
  ...
  <?php if($ENABLE_BULK_WHATSAPP && $rows): ?>
  ...
  <nav class="mt-3">
    <ul class="pagination justify-content-center">
      <?php if($page>1): ?>
      <li class="page-item">
        <a class="page-link" href="?tab=matched&p=<?=($page-1)?>&status=<?=urlencode($status)?>&nome=<?=urlencode($nome)?>">&laquo; Anterior</a>
      </li>
      <?php endif;?>
      <?php for($i=1;$i<=$totalPages;$i++): ?>
      <li class="page-item <?=($i==$page?'active':'')?>"><a class="page-link" href="?tab=matched&p=<?=$i?>&status=<?=urlencode($status)?>&nome=<?=urlencode($nome)?>"><?=$i?></a></li>
      <?php endfor;?>
      <?php if($page<$totalPages): ?>
      <li class="page-item">
        <a class="page-link" href="?tab=matched&p=<?=($page+1)?>&status=<?=urlencode($status)?>&nome=<?=urlencode($nome)?>">Próximo &raquo;</a>
      </li>
      <?php endif;?>
    </ul>
  </nav>
...
<?php elseif($tab==='import'): ?>
  ...
  <form method="POST" action="?action=import_process&tab=import">
    <div class="mb-3">
      <label>Formato:</label>
...
<?php elseif($tab==='logs'): ?>
  ...
  <?php
    if(!tableExists($pdo,'whatsapp_logs')){
        echo "<p class='text-danger'>A tabela <code>whatsapp_logs</code> não existe. Crie-a para registrar os envios.</p>";
    } else {
        ...
        <nav class="mt-2">
          <ul class="pagination justify-content-center">
            <?php if($page>1): ?>
            <li class="page-item">
              <a class="page-link" href="?tab=logs&p=<?=($page-1)?>">&laquo; Anterior</a>
            </li>
            <?php endif;?>
            <?php for($i=1;$i<=$totalPages;$i++): ?>
            <li class="page-item <?=($i==$page?'active':'')?>"><a class="page-link" href="?tab=logs&p=<?=$i?>"><?=$i?></a></li>
            <?php endfor;?>
            <?php if($page<$totalPages): ?>
            <li class="page-item">
              <a class="page-link" href="?tab=logs&p=<?=($page+1)?>">Próximo &raquo;</a>
            </li>
            <?php endif;?>
          </ul>
        </nav>
...
<?php elseif($tab==='config'): ?>
  ...
  <?php
    if(!tableExists($pdo,'settings')){
        echo "<p class='text-danger'>A tabela <code>settings</code> não existe no BD.</p>";
    } else {
        ...
        <form method="POST" action="?action=test_config_send&tab=config" class="row g-3 align-items-end">
          <div class="col-auto">
        ...
        <p class="mt-4">
          <a href="?tab=dashboard" class="btn btn-secondary">&larr; Voltar ao Dashboard</a>
        </p>
        <?php
    }
  ?>
...
function reenvioWhatsApp(orderId){
  ...
  if(confirm("Deseja enviar ou reenviar o código via WhatsApp com link:\n"+baseLink+"?")){
    window.location="?action=send_whatsapp&id="+orderId+"&tab=matched&base="+encodeURIComponent(baseLink);
  }
}
</script>
</body>
